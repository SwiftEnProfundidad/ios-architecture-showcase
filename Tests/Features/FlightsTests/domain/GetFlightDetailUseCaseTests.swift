...
struct GetFlightDetailUseCaseTests {
    ...
    @Test("Given flight and weather requests, when detail is loaded, then both are started concurrently using async let")
    ...
    @Test("Given weather loading fails, when detail loading completes, then flight detail is returned without weather")
