...
struct CoverageSummary: Equatable {
    ...
    let totalLines: Int
    let productionFiles: Int

    func meets(threshold: Double) -> Bool {
        percentage + 0.000_000_001 >= threshold
    }

    func formatted(threshold: Double) -> String {
...
enum CoverageGateEvaluatorError: Error, Equatable {
    case fixtureNotFound(String)
    case invalidReport
    case missingProductionFiles
}

enum CoverageGateEvaluator {
    static func loadSummary(from reportURL: URL) throws -> CoverageSummary {
